...
uint8_t USART_Rx_State_UWB = 0;
uint8_t USART_Rx_Index_UWB = 0;
uint8_t USART_Rx_Complete_Flag_UWB = 0;
/*UWB Variables*/
...
{
  UWB_X = (float)((A0_A1_X + A0_A2_X + A1_A3_X + A2_A3_X) / 4);
  UWB_Y = (float)((A0_A1_Y + A0_A2_Y + A1_A3_Y + A2_A3_Y) / 4);
}

/**
 * @brief       获取UWB串口接收数据包标志位
 * @param       无
...
{
  if(USART_Rx_Complete_Flag_UWB == 1)
  {
    USART_Rx_Complete_Flag_UWB = 0;
