...
void USART6_Receive_Data(void)
{
  ...
  else
  {
    ...
    {
      Receive_Data.Receive_Data_Struct.Vcx_Int = -((Receive_Data.Receive_Data_Buf[2] >> 4) & 0x03);
    }
    else
    {
      Receive_Data.Receive_Data_Struct.Vcx_Int = +((Receive_Data.Receive_Data_Buf[2] >> 4) & 0x03);
    }
    Receive_Data.Receive_Data_Struct.Vcx_Dec = (float)((uint16_t)(Receive_Data.Receive_Data_Buf[2] & 0x0F) << 8 | Receive_Data.Receive_Data_Buf[3]) / 1000.0f;
    Robot_Vcx = Receive_Data.Receive_Data_Struct.Vcx_Int + Receive_Data.Receive_Data_Struct.Vcx_Dec;
    
    // Vcy
    if(Receive_Data.Receive_Data_Buf[4] & 0x80)
    {
      Receive_Data.Receive_Data_Struct.Vcy_Int = -((Receive_Data.Receive_Data_Buf[4] >> 4) & 0x03);
    }
    else
    {
      Receive_Data.Receive_Data_Struct.Vcy_Int = +((Receive_Data.Receive_Data_Buf[4] >> 4) & 0x03);
    }
    Receive_Data.Receive_Data_Struct.Vcy_Dec = (float)((uint16_t)(Receive_Data.Receive_Data_Buf[4] & 0x0F) << 8 | Receive_Data.Receive_Data_Buf[5]) / 1000.0f;
    Robot_Vcy = Receive_Data.Receive_Data_Struct.Vcy_Int + Receive_Data.Receive_Data_Struct.Vcy_Dec;
    
    // Wc
    if(Receive_Data.Receive_Data_Buf[6] & 0x80)
    {
      Receive_Data.Receive_Data_Struct.Wc_Int = -((Receive_Data.Receive_Data_Buf[6] >> 4) & 0x03);
    }
    else
    {
      Receive_Data.Receive_Data_Struct.Wc_Int = +((Receive_Data.Receive_Data_Buf[6] >> 4) & 0x03);
    }
    Receive_Data.Receive_Data_Struct.Wc_Dec = (float)((uint16_t)(Receive_Data.Receive_Data_Buf[6] & 0x0F) << 8 | Receive_Data.Receive_Data_Buf[7]) / 1000.0f;
    Robot_Wc = Receive_Data.Receive_Data_Struct.Wc_Int + Receive_Data.Receive_Data_Struct.Wc_Dec;
  }
}
